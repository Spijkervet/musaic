...
from v9.Data.DataGeneratorsLeadMeta import CombinedGenerator
...
#    save_dir = "_".join([*save_dir[0:3], *save_dir[3].split(":")[:2]])
    ...
    # META
    meta_embedder = MetaEmbedding.from_saved_custom("/".join([top_dir, save_dir, "meta"]), compile_now=True)
    meta_embed_size = meta_embedder.embed_size
    meta_predictor = MetaPredictor.from_saved_custom("/".join([top_dir, save_dir, "meta"]), compile_now=True)
    meta_predictor.freeze()

    # CHANGE
    music_dir = "/home/jspijkervet/eurovision/Data/eurovisionMidi/"
    ...
    # COMBINED NETWORK
    print(context_size, m, meta_embed_size)
    # exit(0)
